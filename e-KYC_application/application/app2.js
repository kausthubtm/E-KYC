...
async function main() {
	try {
		...
		await registerAndEnrollUser(caClient, wallet, mspOrg2, org1UserId, 'org2.department1');
		...
		try {
			...
			rl.question("\n\n\n\n\n\n\nChoose a query :\n 1 : read all clients\n 2 : read a particular client\n 3 : create a new client\n 4 : Update a client\n 5 : Delete a client\n\n\n\nYour preference : ", async function(type){
				...
				if(type == '1') {
					try {
						console.log('\n--> Evaluate Transaction: GetAllCustomers, function returns all the current clients on the ledger');
				...
				if(type == '2') {
					...
					prompt.get(['id'], async function (err, result) {
						...
						if(result.id) {
							try {
								console.log('\n--> Evaluate Transaction: ReadCustomer, function returns an client with a given ID');
